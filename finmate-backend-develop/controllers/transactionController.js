...
const {
    ...
    DELETE_SUCCESS, INSERT_SUCCESS,UPDATE_SUCCESS
  ...
  try{
    ...
    if (result.length > 0) {
      ...
      result.map((item) => {
        ...
        res.status(200).json({
        status: 'SUCCESS',
        data: {data:result, totalIncome:income, totalExpense:expense, totalSavings:savings, totalBalance:income - expense - savings},
        });
    } else {
        res.status(200).json({
...
exports.addTransactionDetails = async (req, res) => {
  ...
  try{
    let result = await addTransactionDetails(connectionPromise, transaction, req.user);
    if (result.affectedRows > 0) {
      res.status(200).json({
        status: 'SUCCESS',
      ...
      res.status(500).json({
        status: 'ERROR',
        message: err?err.message:"Failed to save transaction details, Please try again!"
      });
    }
  }catch(err){
    console.log(err)
...
exports.updateTransactionDetails = async (req, res) => {
  ...
  transaction.date = transaction.date.split("/").reverse().join("/");
  try{
    let result = await updateTransactionDetails(connectionPromise, transaction, req.user);
    if (result.affectedRows > 0) {
      res.status(200).json({
        status: 'SUCCESS',
      ...
      res.status(500).json({
        status: 'ERROR',
        message: err?err.message:"Failed to update transaction details, Please try again!"
      });
    }
  }catch(err){
    res.status(500).json({
