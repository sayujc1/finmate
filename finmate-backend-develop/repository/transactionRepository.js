exports.fetchTransactionDetailsByType = async (
  connectionPromise,
  type,
  month,
  year,
...
exports.deleteTransactionDetailsById = async (connectionPromise, tid, user) => {
  let result = await connectionPromise(
    `DELETE FROM transactions WHERE t_id = '${tid}' AND user_id = '${user.user_id}'`
  );
  return result;
};
...
exports.updateTransactionDetails = async (
  connectionPromise,
  transaction,
  user
) => {
  let result = await connectionPromise(
    `UPDATE transactions SET type = '${transaction.type}', amount = '${transaction.amount}', date = '${transaction.date}', description = '${transaction.description}', category = '${transaction.category}', remarks = '${transaction.remarks}', category_others = '${transaction.category_others}' WHERE t_id = '${transaction.t_id}' AND user_id = '${user.user_id}'`
  );
  return result;
};

exports.fetchTransactionDetails = async (
  connectionPromise,
  dateType,
  dateStart,
  dateEnd,
  ...
  if (dateType === "date") {
    ...
    query += ` AND WEEK(date) = '${
      dateStart.split("-")[1]
    }' AND YEAR(date) = '${dateStart.split("-")[0]}'`;
    ...
    query += ` AND QUARTER(date) = '${
      dateStart.split("-")[1]
    }' AND YEAR(date) = '${dateStart.split("-")[0]}'`;
  } else {
    query += ` AND date BETWEEN '${dateStart
      .split("/")
      .reverse()
...
exports.fetchRecentTransactionDetails = async (
  connectionPromise,
  limit,
  user
) => {
  let result = await connectionPromise(
    `SELECT * FROM transactions WHERE user_id = '${user.user_id}' ORDER BY last_updated_time DESC LIMIT ${limit}`
  );
  return result;
};
