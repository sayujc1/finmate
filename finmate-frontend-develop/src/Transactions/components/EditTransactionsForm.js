      ...
      .then(async () => {
        ...
        isEdit &&
          (await UpdateForm(
            ...
            transactionReducer.editData,
            addForm,
            transactionReducer?.editData?.type,
            dispatch
          )
            .then(response => {
              notification.success({
                message:
                  "Successfully Updated " + transactionReducer?.editData?.type,
              });
              handleReset();
            ...
            .catch(err => {
              notification.error({
                message:
                  "Failed to Update " + transactionReducer?.editData?.type,
                description: err,
                duration: 8,
              });
            }));
      ...
      <div className={EditTransactionsFormCss.editForm}>
          ...
          title={
            !isEdit && (
              <div style={{ textAlign: "center" }}>
                Add {`${transactionReducer?.editData?.type}`}
              </div>
            )
          }
          bordered={false}
        ...
        >
          <>
            ...
            <>
              <Popconfirm
                title={`Are you sure you want to ${isEdit ? "Edit" : "Add"} ${
                  transactionReducer?.editData?.type
                }?`}
